    ...
    <>
      ...
      <Section className="pt-32 pb-12">
        <Container size="narrow">
          ...
          <div className="mt-8">
            <span className="text-[color:var(--text-muted)] text-sm">February 2026 &middot; 14 min read</span>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[rgba(255, 136, 0,0.15)]">
                      <th className="text-left py-3 px-4 text-[color:var(--text-muted)] font-medium">Campaign Type</th>
                      <th className="text-right py-3 px-4 text-[color:var(--accent-bright)] font-medium">Avg ROAS</th>
                      <th className="text-right py-3 px-4 text-[color:var(--accent-primary)] font-medium">Avg CPC</th>
                      <th className="text-right py-3 px-4 text-[color:var(--accent-primary)] font-medium">Conv Rate</th>
                      <th className="text-left py-3 px-4 text-[color:var(--text-muted)] font-medium">Best For</th>
                    </tr>
                  </thead>
                  <tbody>
                    {campaignTypeBenchmarks.map((row, i) => (
                      <tr key={row.type} className={i < campaignTypeBenchmarks.length - 1 ? 'border-b border-[rgba(255, 136, 0,0.08)]' : ''}>
                        ...
                        <td className="py-3 px-4 text-right text-[color:var(--text-primary)]">{row.avgRoas}</td>
                        <td className="py-3 px-4 text-right text-[color:var(--text-primary)]">{row.avgCpc}</td>
                        <td className="py-3 px-4 text-right text-[color:var(--text-primary)]">{row.avgConvRate}</td>
                        <td className="py-3 px-4 text-[color:var(--text-muted)] text-xs">{row.bestFor}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <Section className="pb-12">
        <Container size="narrow">
          <FadeUp>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[rgba(255, 136, 0,0.15)]">
                      ...
                      <th className="text-right py-3 px-4 text-[color:var(--accent-bright)] font-medium">Search ROAS</th>
                      <th className="text-right py-3 px-4 text-[color:var(--accent-primary)] font-medium">Avg CPC</th>
                      <th className="text-right py-3 px-4 text-[color:var(--accent-primary)] font-medium">Conv Rate</th>
                    </tr>
                  </thead>
                  <tbody>
                    {searchBenchmarks.map((row, i) => (
                      <tr key={row.industry} className={i < searchBenchmarks.length - 1 ? 'border-b border-[rgba(255, 136, 0,0.08)]' : ''}>
                        ...
                        <td className="py-3 px-4 text-right text-[color:var(--text-primary)]">{row.convRate}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <p className="text-[color:var(--text-muted)] text-xs mt-3">Data based on analysis of $62M in Google Ads spend across 420+ accounts, 2025-2026.</p>
          </FadeUp>
        </Container>
      </Section>
